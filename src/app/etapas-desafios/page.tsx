                            ...
                            <button onClick={() => handleOnClick(etapa.etapaIndice)} disabled={etapa.estado == false} title={`Etapa ${etapa.etapaIndice}`} className={`relative size-44 rounded-3xl ${etapa.estado == true ? 'hover:transition-all hover:duration-100 hover:underline hover:underline-offset-4 decoration-verdeBX hover:ring-8 hover:ring-azulBX hover:ring-opacity-20 hover:bg-blue-600 hover:bg-opacity-20 active:ring-8 active:ring-laranjaBX active:ring-opacity-70' : ''}  ${indice == etapa.etapaIndice ? 'scale-[1.13] shadow-sky-900 shadow-lg ' : ''}`}>
                                {
                                    etapa.estado == false ? //caso a etapa ainda não tenha ocorrido:
                                        ...
                                        <div>
                                            ...
                                            <h2 className={`mt-24 ${poppins.className} text-white text-2xl [text-shadow:_0_1.3px_0_var(--tw-shadow-color)] shadow-gray-600 underline underline-offset-4 decoration-verdeBX decoration-[3px]`}><b>Etapa {etapa.etapaIndice}</b></h2>
